<template>
  <div>
     <li :class="['node',node.children?'':'node-leaf']" v-for="node in data" :key='node.index'>
        <div @click="node.open = !node.open">
            <div v-if="node.children" @click="loadItem(node)" class="li-hover-item" :style="{ paddingLeft: ( node.level*20 + 17 ) + 'px' }">
